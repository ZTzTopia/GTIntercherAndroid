...
class CSTSWebViewClient extends WebViewClient {
    ...
    public void onPageFinished(WebView webView, String str) {
        ...
        webView.loadUrl(
            "javascript:function csts_onTicketCreationResult(wasTicketCreated, message) {window.location.href = 'ticket://'+(wasTicketCreated?1:0)+'/'+message; };");
    }
    ...
    public void onReceivedError(
        WebView webView, WebResourceRequest webResourceRequest, WebResourceError webResourceError
    ) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            Log.e(
                "csts",
                "onReceivedError [" + ((Object) webResourceError.getDescription()) + "] : " + webResourceRequest.getUrl()
            );
        }
    }
    ...
    public void onReceivedHttpError(
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            Log.e(
                "csts",
                "onReceivedHttpError [" + webResourceResponse.getStatusCode() + "] : " + webResourceRequest.getUrl()
            );
        }
    }
    ...
    public boolean shouldOverrideUrlLoading(WebView webView, String str) {
        ...
        if (str.equals("exit://")) {
            ...
            if (str.startsWith("ticket://")) {
                ...
                Log.v(
                    "cstslog",
                    "Ticket creation status: " + ticketStatus + " detail: " + str.substring(11)
                );
                return true;
            }
