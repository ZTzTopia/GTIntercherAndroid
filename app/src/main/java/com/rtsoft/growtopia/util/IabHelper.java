...
package com.rtsoft.growtopia.util;
...
public class IabHelper {
    ...
    public void startSetup(final OnIabSetupFinishedListener listener) {
        ...
        mServiceConn = new ServiceConnection() {
            ...
            public void onServiceConnected(ComponentName name, IBinder service) {
                ...
                try {
                    logDebug("Checking for in-app billing 6 support.");

                    // check for in-app billing v6 support
                    int response = mService.isBillingSupported(6, packageName, ITEM_TYPE_INAPP);
                    if (response != BILLING_RESPONSE_RESULT_OK) {
                        if (listener != null) listener.onIabSetupFinished(new IabResult(response,
                                "Error checking for billing v6 support."));
                    ...
                    if (mSubscriptionUpdateSupported) {
                        ...
                        // check for v6 subscriptions support
                        response = mService.isBillingSupported(6, packageName, ITEM_TYPE_SUBS);
                        if (response == BILLING_RESPONSE_RESULT_OK) {
    ...
    public void launchPurchaseFlow(Activity act, String sku, int requestCode,
                                   ...
                                   String extraParams)
            throws IabAsyncInProgressException {
        launchPurchaseFlow(act, sku, ITEM_TYPE_INAPP, null, requestCode, listener, extraData, extraParams);
    }
    ...
    public void launchSubscriptionPurchaseFlow(Activity act, String sku, int requestCode,
                                               ...
                                               String extraParams)
            throws IabAsyncInProgressException {
        launchPurchaseFlow(act, sku, ITEM_TYPE_SUBS, null, requestCode, listener, extraData, extraParams);
    }
    ...
    public void launchPurchaseFlow(Activity act, String sku, String itemType, List<String> oldSkus,
        ...
        try {
            ...
            bundle.putString(BillingFlowParams.EXTRA_PARAM_KEY_ACCOUNT_ID, extraParams);
            Bundle buyIntentExtraParams = mService.getBuyIntentExtraParams(6, mContext.getPackageName(),
                        sku, itemType, extraData, bundle);
            int response = getResponseCodeFromBundle(buyIntentExtraParams);
            if (response != BILLING_RESPONSE_RESULT_OK) {
                logError("Unable to buy item, Error response: " + getResponseDesc(response));
                flagEndAsync();
                result = new IabResult(response, "Unable to buy item");
            ...
            PendingIntent pendingIntent = buyIntentExtraParams.getParcelable(RESPONSE_BUY_INTENT);
     ...
     */
    public boolean queryInventoryAsync(final boolean querySkuDetails, final List<String> moreItemSkus,
                                    final List<String> moreSubsSkus, final QueryInventoryFinishedListener listener)
            throws IabAsyncInProgressException {
        final Handler handler = new Handler();
        checkNotDisposed();
        if (!checkSetupDoneNoException("queryInventory")) {
            return false;
        }
    ...
    public boolean queryInventoryAsync(QueryInventoryFinishedListener listener)
            throws IabAsyncInProgressException {
        return queryInventoryAsync(true, null, null, listener);
    }
    ...
    void consume(Purchase itemInfo) throws IabException {
        ...
        try {
            ...
            int response = mService.consumePurchase(6, mContext.getPackageName(), token);
            if (response == BILLING_RESPONSE_RESULT_OK) {
    ...
    boolean checkSetupDoneNoException(String operation) {
        if (!mSetupDone) {
            logError("Illegal state for operation (" + operation + "): IAB helper is not set up.");
            return false;
        }
    ...
    int queryPurchases(Inventory inv, String itemType) throws JSONException, RemoteException {
        ...
        do {
            ...
            Bundle ownedItems = mService.getPurchases(6, mContext.getPackageName(),
