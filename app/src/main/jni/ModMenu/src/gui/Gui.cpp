...
namespace gui {
    ...
    void Gui::handle_soft_input() {
        ...
        static bool last_WantTextInput{ false };
        static std::list<ImGuiInputTextState *>last_GetInputState{};
        static ImGuiID last_ActiveId{ 0 };

        // Is we passed the next two frame? yay we can add text to text input.
        if (m_wait_until_next_frame < 2) {
            m_wait_until_next_frame++;
        }

        for (auto &GetInputState : last_GetInputState) {
            if (GetInputState->ID != g.ActiveId) {
                // Just process the current active id.
                continue;
            }

            // Save data for later use. (will optimize later)
            mINI::INIFile file("/storage/emulated/0/Android/data/com.gt.launcher/files/imgui_save.ini");
            mINI::INIStructure ini;
            file.read(ini);

            ini["userdata"]["input_text_" + std::to_string(GetInputState->ID)] = GetInputState->TextA.Data;
            file.generate(ini);
        }

        if (io.WantTextInput && !last_WantTextInput) {
            // Oh the soft input is opened wait until next frame to add text to text input.
            m_wait_until_next_frame = 0;
            ...
            if (state) {
                ...
                if (!found) {
                    last_GetInputState.push_back(state);
                }
            }
        }

        last_WantTextInput = io.WantTextInput;

        if (last_WantTextInput && g.ActiveId != last_ActiveId) {
            utilities::java_wrapper::show_soft_keyboard_input(false, "", false);

            // If another item is focused and want text input,
            // we need to reopen the soft input.
            if (!io.WantTextInput || g.ActiveId != 0) {
                last_WantTextInput = false;
            }
        }
